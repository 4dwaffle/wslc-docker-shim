...
namespace Testcontainers.WslcShim.Http.Tests;
...
public sealed class ShimHttpEndpointTests
{
    ...
    public async Task Full_listener_mutates_ryuk_create_request_before_backend_create()
    {
        ...
        Assert.Contains("DOCKER_HOST=tcp://172.28.48.1:49152", created.Env);
        Assert.DoesNotContain(created.HostConfig.Binds, bind => bind.Contains("/var/run/docker.sock"));
    }
    ...
    public async Task Full_listener_rejects_unsupported_create_settings_instead_of_dropping_them()
    {
        var backend = new RecordingDockerBackend
        {
            CreateContainerException = new UnsupportedDockerCreateOptionException(["HostConfig.Privileged"])
        };
        using var server = await ShimTestServer.CreateAsync(backend);
        var client = server.GetTestClient();
    ...
    private sealed class RecordingDockerBackend : IDockerBackend
    {
        ...
        public List<DockerContainerCreateRequest> CreatedContainers { get; } = [];
        ...
        public UnsupportedDockerCreateOptionException? CreateContainerException { get; init; }

        public Task<DockerCreateContainerResponse> CreateContainerAsync(
            DockerContainerCreateRequest request,
            CancellationToken cancellationToken)
        {
            if (CreateContainerException is not null)
            {
                throw CreateContainerException;
            }

            CreatedContainers.Add(request);
